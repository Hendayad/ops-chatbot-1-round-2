"""Authenticated, rate-limited read-only APIs for at-risk Ops dashboards (PRD F3.3).
...
async def get_atrisk_summary(
    ...
    """Return the at-risk aggregate (counts + per-reason breakdown) for one day.
...
async def get_atrisk_trend(
    ...
    """Return one at-risk aggregate per day for the trailing `days` days.
    ...
    """
    try:
        end_date = get_latest_run_date() or date.today()
        start_date = end_date - timedelta(days=days - 1)
        trend = get_aggregate_trend(start_date, end_date)
        logger.info(
...
async def get_atrisk_learners(
    ...
    """Return the learner_ids currently flagged at_risk for one day.
    ...
    try:
        ...
        learner_ids = get_at_risk_learner_ids(target_date)
        logger.info(
